...
import { settlementOperations, coupleOperations, expenseOperations } from '@/lib/database';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 精算一覧を取得
    const settlements = await settlementOperations.getCoupleSettlements(couple.id, {
      limit,
      offset,
      status,
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!expense_ids || !Array.isArray(expense_ids) || expense_ids.length === 0) {
      return NextResponse.json({ error: '精算対象の支出を選択してください' }, { status: 400 });
    }
    if (!period_start || !period_end) {
      return NextResponse.json({ error: '精算期間は必須です' }, { status: 400 });
    }
    if (new Date(period_end) < new Date(period_start)) {
      return NextResponse.json({ error: '期間の終了日は開始日より後である必要があります' }, { status: 400 });
    }
    ...
    if (validExpenses.length !== expense_ids.length) {
      return NextResponse.json({ error: '無効な支出が含まれています' }, { status: 400 });
    }

    const settledExpenses = validExpenses.filter(exp => exp!.is_settled);
    if (settledExpenses.length > 0) {
      return NextResponse.json({ error: '既に精算済みの支出が含まれています' }, { status: 400 });
    }

    // カップルの支出かチェック
    const invalidExpenses = validExpenses.filter(exp => exp!.couple_id !== couple.id);
    if (invalidExpenses.length > 0) {
      return NextResponse.json({ error: 'アクセス権限のない支出が含まれています' }, { status: 403 });
    }
    ...
    const user2Balance = user2Payments - user2Share;
    ...
    if (settlementAmount === 0) {
      return NextResponse.json({ error: '精算の必要がありません' }, { status: 400 });
    }
